...
struct some_t
{
  ...
  auto check(T&& t) -> bool
  {
    auto ptr = customization::pointer_getter<
        std::decay_t<T>>::template get_pointer<Class>(std::forward<T>(t));
    if (!ptr) {
      return false;
    }
    return match_check(*ptr, m);
  }
...
struct some_t<void, Matcher>
{
  ...
  auto check(T&& t) -> bool
  {
    // If you get an error here, this means that some() without a type is not
    // supported Examples of this are variants and boost::any
    auto ptr =
        customization::pointer_getter<std::decay_t<T>>::get_pointer_no_cast(
            std::forward<T>(t));
    if (!ptr) {
      return false;
    }
...
struct none_t
{
  ...
  auto check(T&& t) -> bool
  {
    ...
    return customization::pointer_getter<std::decay_t<T>>::is_null(
        std::forward<T>(t));
  }
...
inline auto none() -> detail::none_t
{
  return detail::none_t {};
}

inline auto some() -> detail::some_t<void, void>
{
  return detail::some_t<void, void> {};
}

// Make Matcher... a variadic template so it is a worse match than some<T>(T())
template<class... Matcher>
auto some(Matcher&&... m) -> detail::some_t<void, Matcher...>
{
  return detail::some_t<void, Matcher...> {std::forward<Matcher>(m)...};
}

template<class Class, class Matcher>
auto some(Matcher&& m) -> detail::some_t<Class, Matcher>
{
  return detail::some_t<Class, Matcher> {std::forward<Matcher>(m)};
}
template<class Class>
auto some() -> detail::some_t<Class, void>
{
...
{
  using type =
      cat_tuple_t<std::tuple<Class>,
                  typename get_some_classes<std::tuple<Rest...>>::type>;
};
...
struct all_in<SomeClasses, First, Rest...>
{
  ...
  static const bool value = not_in_match_asserter<myvalue, First>::value
      && all_in<SomeClasses, Rest...>::value;
};
template<class SomeClasses, class First>
struct all_in<SomeClasses, First>
{
  static const bool myvalue = type_in_tuple<First, SomeClasses>::value;
  // static_assert(type_in_tuple<First, SomeClasses>::value, "This class is not
  // in the match");
  static const bool value = not_in_match_asserter<myvalue, First>::value;
};
...
struct some_exhaustiveness
{
  ...
  struct type
  {
    ...
    using helper = detail::some_exhaustiveness_helper<v, ArgTypes, Types...>;
